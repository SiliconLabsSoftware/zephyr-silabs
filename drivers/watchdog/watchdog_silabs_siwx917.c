...
static uint32_t siwx917_wdt_delay_from_hw(uint8_t value, int clock_frequency)
{
...
static uint8_t siwx917_wdt_delay_to_hw(uint32_t delay, int clock_frequency)
{
...
static int siwx917_wdt_install_timeout(const struct device *dev, const struct wdt_timeout_cfg *cfg)
{
	struct siwx917_wdt_data *data = dev->data;
	...
	if (cfg->window.max > siwx917_wdt_delay_from_hw(SIWX91X_WDT_SYSTEM_RESET_TIMER_MASK,
	...
	case WDT_FLAG_RESET_CPU_CORE:
		...
		data->delay_reset = siwx917_wdt_delay_to_hw(cfg->window.max, data->clock_frequency);
	...
	case WDT_FLAG_RESET_NONE:
		...
		data->delay_irq = siwx917_wdt_delay_to_hw(cfg->window.max, data->clock_frequency);
		if (cfg->callback != NULL) {
...
static int siwx917_wdt_setup(const struct device *dev, uint8_t options)
{
	const struct siwx917_wdt_config *config = dev->config;
	struct siwx917_wdt_data *data = dev->data;
...
static int siwx917_wdt_disable(const struct device *dev)
{
	const struct siwx917_wdt_config *config = dev->config;
	struct siwx917_wdt_data *data = dev->data;
...
static int siwx917_wdt_feed(const struct device *dev, int channel_id)
{
	const struct siwx917_wdt_config *config = dev->config;
	struct siwx917_wdt_data *data = dev->data;
...
static void siwx917_wdt_isr(const struct device *dev)
{
	const struct siwx917_wdt_config *config = dev->config;
	struct siwx917_wdt_data *data = dev->data;
...
static int siwx917_wdt_init(const struct device *dev)
{
	const struct siwx917_wdt_config *config = dev->config;
	struct siwx917_wdt_data *data = dev->data;
	int ret;
...
static DEVICE_API(wdt, siwx917_wdt_driver_api) = {
	.setup = siwx917_wdt_setup,
	.disable = siwx917_wdt_disable,
	.install_timeout = siwx917_wdt_install_timeout,
	.feed = siwx917_wdt_feed,
};

#define SIWX917_WDT_INIT(inst)                                                                     \
	static struct siwx917_wdt_data siwx917_wdt_data_##inst;                                    \
	static void siwx917_wdt_irq_configure_##inst(void)                                         \
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQ(inst, irq), DT_INST_IRQ(inst, priority), siwx917_wdt_isr,  \
			    DEVICE_DT_INST_GET(inst), 0);                                          \
		irq_enable(DT_INST_IRQ(inst, irq));                                                \
	}                                                                                          \
	static const struct siwx917_wdt_config siwx917_wdt_config_##inst = {                       \
		.reg = (MCU_WDT_Type *)DT_INST_REG_ADDR(inst),                                     \
		.clock_dev = DEVICE_DT_GET(DT_INST_CLOCKS_CTLR(inst)),                             \
		.clock_subsys = (clock_control_subsys_t)DT_INST_PHA(inst, clocks, clkid),          \
		.irq_config = siwx917_wdt_irq_configure_##inst,                                    \
	};                                                                                         \
	DEVICE_DT_INST_DEFINE(inst, &siwx917_wdt_init, NULL, &siwx917_wdt_data_##inst,             \
			      &siwx917_wdt_config_##inst, PRE_KERNEL_1,                            \
			      CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &siwx917_wdt_driver_api);
