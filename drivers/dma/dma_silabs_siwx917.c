...
struct dma_siwx917_config {
	...
	uint32_t sram_desc_addr;     /* SRAM Address for UDMA Descriptor Storage */
...
struct dma_siwx917_data {
	...
	uint8_t dma_rom_buff[sizeof(RSI_UDMA_DATACONTEXT_T)]; /* Buffers to store UDMA handle */
							      /* related information */
};
			      ...
			      UDMA_Channel_Info *channel_info)
{
	...
	UDMA_RESOURCES udma_resources = {
		...
		.desc = (RSI_UDMA_DESC_T *)cfg->sram_desc_addr,
	};
	...
	status = UDMAx_ChannelConfigure(
		&udma_resources, (uint8_t)channel, (uint32_t)(config->head_block->source_address),
		(uint32_t)(config->head_block->dest_address), config->head_block->block_size,
		channel_control, &channel_config, NULL, channel_info, udma_handle);
	if (status) {
		return -EIO;
	}
	return 0;
				 ...
				 struct dma_config *config)
{
	...
	int status;
	RSI_UDMA_HANDLE_T udma_handle = data->dma_rom_buff;
			      ...
			      uint32_t dst, size_t size)
{
	...
	uint32_t length;
	RSI_UDMA_HANDLE_T udma_handle = data->dma_rom_buff;
	RSI_UDMA_DESC_T *udma_table = (RSI_UDMA_DESC_T *)cfg->sram_desc_addr;
...
static int dma_siwx917_start(const struct device *dev, uint32_t channel)
{
	...
	if ((udma_table[channel].vsUDMAChaConfigData1.srcInc != UDMA_SRC_INC_NONE) &&
		...
		cfg->reg->CHNL_SW_REQUEST |= SET_BIT(channel);
	}
	return 0;
}
...
#define SIWX917_DMA_INIT(inst)                                                                     \
	...
	static const struct dma_siwx917_config dma##inst##_cfg = {                                 \
		...
		.sram_desc_addr = DT_INST_PROP(inst, silabs_sram_desc_addr),                       \
