...
static bool pinctrl_siwx917_valid_mode(uint8_t mode)
{
	return (mode < MODE_COUNT);
}

static void pinctrl_siwx917_set(uint8_t port, uint8_t pin, uint8_t ulppin, uint8_t mode,
				uint8_t ulpmode, uint8_t pad)
{
	...
	if (pinctrl_siwx917_valid_mode(mode)) {
		GPIO->PIN_CONFIG[((port << 4) | pin)].GPIO_CONFIG_REG_b.MODE = mode;
	}

	if (pinctrl_siwx917_valid_mode(ulpmode)) {
		if (pinctrl_siwx917_valid_mode(mode) && ulpmode != HP_PERIPHERAL_ON_ULP_PIN) {
...
int pinctrl_configure_pins(const pinctrl_soc_pin_t *pins, uint8_t pin_cnt, uintptr_t reg)
{
	...
	for (i = 0; i < pin_cnt; i++) {
		pinctrl_siwx917_set(pins[i].port, pins[i].pin, pins[i].ulppin, pins[i].mode,
